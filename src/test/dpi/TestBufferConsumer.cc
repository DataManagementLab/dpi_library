...
void TestBufferConsumer::testSegmentIterator()
{
  ...
  BufferHandle *buffHandle = new BufferHandle(bufferName, 1, numberSegments, 1, Config::DPI_SEGMENT_SIZE - sizeof(Config::DPI_SEGMENT_HEADER_t)); //Create 1 less segment in ring to test BufferWriterBW creating a segment on the ring
...
void TestBufferConsumer::testBufferIteratorFourAppender_NotInterleaved()
{
  ...
  CPPUNIT_ASSERT(m_regClient->registerBuffer(new BufferHandle(bufferName, nodeId, segPerClient, numClients, Config::DPI_SEGMENT_SIZE - sizeof(Config::DPI_SEGMENT_HEADER_t))));
...
//     CPPUNIT_ASSERT_EQUAL_MESSAGE("CanConsume did not match expected", false, Config::DPI_SEGMENT_HEADER_FLAGS::isConsumable(header->segmentFlags));
//     CPPUNIT_ASSERT_EQUAL_MESSAGE("CanWrite did not match expected", true, Config::DPI_SEGMENT_HEADER_FLAGS::isWriteable(header->segmentFlags));
//   }
